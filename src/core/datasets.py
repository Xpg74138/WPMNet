...
class CustomDataset(Dataset):
    def __init__(self, list_path, class_names, model_type,img_size=416, transform=None, cache_file=None):
        ...
        if cache_file is None:
            ...
            self.depth_images = [self.load_image(f, mode='I', dtype='uint8') for f in tqdm(self.depth_img_files, desc="Loading Depth images")]
        elif cache_file:
    ...
    def load_image(self, path, mode, dtype=None):
        """加载并调整图像大小"""
        try:
            img = Image.open(path).convert(mode).resize((self.img_size, self.img_size))
            img = np.array(img, dtype=dtype)  # 确保转换为 NumPy 数组
        except:
            img=io.imread(path) #Mr.Ma的数据
        return img
    ...
    def preprocess_and_cache(self):
        ...
        for idx in tqdm(range(len(self.rgb_img_files)), desc="Caching images"):
            ...
            depth_img = self.load_image(self.depth_img_files[idx], mode='I', dtype='uint8')
            # 将深度图像扩展为3通道
    ...
    def __getitem__(self, index):
        ...
        if self.transform:
            augmented = self.transform(image=rgb_img, depth=depth_img)  # 关键：同时传入两种数据
